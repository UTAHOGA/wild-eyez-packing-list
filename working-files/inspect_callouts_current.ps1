...
for($i=1;$i -le $doc.Shapes.Count;$i++){
 ...
 if($page -in 2,3,4,6,7){
  $txt=ShapeText $s; if($txt.Length -gt 120){$txt=$txt.Substring(0,120)}
  $type='';$children=0;$fill='';$line='';$lw='';$vis=''
  try{$type=$s.Type;if($type -eq 6){$children=$s.GroupItems.Count}}catch{}
  try{$fill=$s.Fill.ForeColor.RGB}catch{}; try{$line=$s.Line.ForeColor.RGB;$lw=$s.Line.Weight;$vis=$s.Line.Visible}catch{}
  if($txt -match 'PRIORITY|BE READY|PROFESSIONAL|PRO TIP|AMMUNITION|HOW TO USE|MOST COMMON' -or ($s.Left -gt 35 -and $s.Left -lt 60 -and $s.Width -gt 500 -and $s.Height -gt 40)){
    ...
    if($type -eq 6){
      for($j=1;$j -le $s.GroupItems.Count;$j++){
        $c=$s.GroupItems.Item($j); $ct=ShapeText $c; if($ct.Length -gt 100){$ct=$ct.Substring(0,100)}
